...
# Hardset Files
bolton_bick_vars <- "/Volumes/bolton/Active/Protected/Annotation_Files/bick.bolton.vars3.txt"
cosmic_hotspots <- "/Users/irenaeuschan/Documents/Irenaeus/data/COSMIC.heme.myeloid.hotspot.tsv"
gene_list <- "/Volumes/bolton/Active/Protected/Annotation_Files/oncoKB_CGC_pd_table_disparity_KB_BW.csv"
...
final <- final %>% FilterSequencingArtifacts()
final <- final %>% FilterGermineVariants()
final <- final %>% NSamples()
final <- final %>% AddNearColumns()
final <- final %>% ProportionalRecurrentFilter()
final <- final %>% PutativeDriver()
final <- final %>% Review()
...
FilterSequencingArtifacts <- function(df) {
  ...
  df <- df %>% dplyr::filter(as.logical(all_fp_pass_XGB))
  
  # Remove any weird Lofreq N calls
  df <- df %>% dplyr::filter(ifelse(grepl('N', ALT), FALSE, TRUE))
  df <- df %>% dplyr::filter(ifelse(grepl('N', REF), FALSE, TRUE))
  
  # Remove Off-Target & Introns          
  df <- df %>% dplyr::filter(Consequence_VEP != "")
  
  # Remove Variants that Failed PoN
  # Bonferroni Corrected p-value for Fisher's Exact Test (0.05 / Size of BED Panel)
  pon_pvalue = 2.114164905e-6        # Archer Panel
  #pon_pvalue = 8.85582713e-7        # MGI Pilot
  df <- df %>% mutate(pon_pvalue_Mutect_Raw = ifelse(is.na(pon_pvalue_Mutect_Raw), 0, pon_pvalue_Mutect_Raw),
  ...
  df <- df %>% mutate(pon_FP_pass_XGB = ifelse(pon_pvalue_Mutect_Raw <= pon_pvalue & pon_pvalue_Lofreq_Raw <= pon_pvalue & pon_pvalue_Vardict_Raw <= pon_pvalue, TRUE, FALSE))
  df <- df %>% filter(pon_FP_pass_XGB == TRUE)
  ...
  # - BCBIO
  df <- df %>% filter(as.logical(pon_FP_pass_XGB), 
  ...
  df <- df %>% filter(FP_Filter_DETP20_XGB == 0,
  ...
  df$average_AF <- df %>% dplyr::select(gt_AF_Mutect, gt_AF_Lofreq, gt_AF_Vardict) %>% rowMeans(., na.rm=TRUE)
  df$average_AD <- df %>% dplyr::select(gt_AD_alt_Mutect, gt_AD_alt_Lofreq, gt_AD_alt_Vardict) %>% rowMeans(., na.rm=TRUE)
  
  # Filter out variants lower than 0.001 VAF because hard to tell the difference between artifact and real
  df <- df %>% filter(average_AF >= 0.001)
  
  # Filter out samples that failed MCV4 if it had an Alt Depth above 5
  df <- df %>% filter(ifelse(FP_Filter_MVC4_XGB == 1 & average_AD > 5, FALSE, TRUE))
  
  # Filter out samples that failed SB1 only if it had an Alt Depth above 10
  df <- df %>% filter(ifelse(FP_Filter_SB1_XGB == 1 & average_AD > 10, FALSE, TRUE))
  ...
  df <- df %>% dplyr::filter(as.logical(PASS_BY_1))
  # Filter our non PD variants
  df <- df %>% dplyr::filter(ch_pd2 >= 1)
  # Filter out Low VAF Mutect Calls
  df <- df %>% filter(!(CALL_BY_CALLER == "mutect" & average_AF < 0.01))
  # Throw out Silent Mutations
  df <- df %>% filter(Consequence_VEP != "synonymous_variant")
  
  min_alt <- 5
  # Strand Bias Filter - 90/10
  df <- df %>% rowwise() %>% mutate(pass_strand_bias = ifelse(sum(pass_strand_bias_Mutect, pass_strand_bias_Lofreq, pass_strand_bias_Vardict, na.rm = TRUE) >= 1, TRUE, FALSE))
  # Strand Bias Filter - Minimum Counts
  df <- df %>% mutate(enough_strand_evidence_Mutect = ifelse(gt_AD_alt_Mutect >= min_alt & (AltFwd_Mutect_Raw == 0 | AltRev_Mutect_Raw == 0), 0, 1),
  ...
  df <- df %>% rowwise() %>% mutate(pass_strand_evidence = ifelse(sum(enough_strand_evidence_Mutect, enough_strand_evidence_Lofreq, enough_strand_evidence_Vardict, na.rm = TRUE) >= 1, TRUE, FALSE))
  df <- df %>% filter(pass_strand_evidence == TRUE)
  # Min Alt Count
  df <- df %>% mutate(min_alt_count_Mutect = ifelse((AltFwd_Mutect_Raw+AltRev_Mutect_Raw) < min_alt, 0, 1))
  df <- df %>% mutate(min_alt_count_Lofreq = ifelse((AltFwd_Lofreq_Raw+AltRev_Lofreq_Raw) < min_alt, 0, 1))
  df <- df %>% mutate(min_alt_count_Vardict = ifelse((AltFwd_Vardict_Raw+AltRev_Vardict_Raw) < min_alt, 0, 1))
  df <- df %>% rowwise() %>% mutate(pass_min_alt_count = ifelse(sum(min_alt_count_Mutect, min_alt_count_Lofreq, min_alt_count_Vardict, na.rm = TRUE) >= 1, TRUE, FALSE))
  df <- df %>% filter(pass_min_alt_count == TRUE) %>% dplyr::select(-min_alt_count_Mutect, -min_alt_count_Lofreq, -min_alt_count_Vardict)
  ...
  #rescue <- df %>% df(!as.logical(pass_homopolymer_filter))
  #df <- df %>% filter(!(as.logical(pass_homopolymer_filter) == FALSE & sourcetotalsloci_XGB == 0))
  ...
  df <- df %>% dplyr::filter(ifelse(CALL_BY_1 == TRUE & (nchar(REF) >= 20 | nchar(ALT) >= 20) & PINDEL_MATCH == "", FALSE, TRUE))
  
  return(df)
}

FilterGermineVariants <- function(df) {
  # If Median VAF >= 35% and NEVER REPORTED in B/B or COSMIC
  # Calculate the Median VAF per Variant
  df <- df %>% left_join(., df %>% group_by(key) %>% summarise(median_VAF = median(average_AF), n_median = n()))
  df <- df %>% filter(!(median_VAF >= 0.35 & n_median > 1 & sourcetotalsc_XGB == 0 & CosmicCount == 0))
  # Keep variants that are below our gnomAD VAF filter (<= gnomAD Population VAF of 0.0005)
  df <- df %>% dplyr::rename(pass_max_sub_gnomAD_AF = max_gnomAD_AF)
  df <- df %>% rowwise() %>% mutate(max_sub_gnomAD_AF = max(max_gnomAD_AF_VEP, max_gnomADe_AF_VEP, max_gnomADg_AF_VEP, na.rm = TRUE)) %>% ungroup()
  df$max_pop_gnomAD_AF <- df %>% dplyr::select(gnomAD_AF_VEP, gnomADe_AF_VEP, gnomADg_AF_VEP) %>% apply(., 1, function(x){max(x, na.rm = T)})
  df <- df %>% mutate(comp_germline = ifelse(max_pop_gnomAD_AF <= 0.0005 | (key == 'chr20 32434638 A>AG' & average_AF >= 0.05) | (key == 'chr20 32434638 A>AGG' & average_AF >= 0.05), 0, 1))
  df <- df %>% mutate(comp_germline = ifelse(average_AF >= 0.35 & (sourcetotalsc_XGB == 0 & sourcetotalsp_XGB == 0 & CosmicCount == 0), 1, comp_germline))
  df <- df %>% mutate(comp_germline = ifelse(average_AF >= 0.35 & Gene %in% c("DNMT3A", "TET2", "ASXL1", "PPM1D") & VariantClass %in% c("frameshift_variant", "stop_lost", "stop_gained", "transcript_ablation"), 0, comp_germline))
  #df <- df %>% mutate(comp_germline = ifelse(average_AF >= 0.25 & average_AF < 0.35 & sourcetotalsc_XGB < 5 & CosmicCount < 25 & max_sub_gnomAD_AF > 0.0001, 1, comp_germline))
  df <- df %>% mutate(comp_germline = ifelse(median_VAF >= 0.35 & average_AF >= 0.25, 1, comp_germline))
  df <- df %>% filter(comp_germline == 0)
  return(df)
}

NSamples <- function(df) {
  # Recurrent Filter Parameters
  # 80 Samples have R882H Mutation in ArcherDX Dataset
  count_threshold<- max(ceiling(length(unique((df$SN_TAG)))*0.06), 5)
  bb_count_threshold<- max(ceiling(length(unique((df$SN_TAG)))*0.03), 2)
  n_samples_min_vaf <- 0.001
  n_samples_min_count <- 5
  
  # N_samples
  df <- df %>% group_by(key) %>% mutate(nsamples = n()) %>% ungroup()
  df <- df %>% left_join(., df %>% 
                           dplyr::filter(average_AF >= n_samples_min_vaf) %>%
                           group_by(key) %>%
                           mutate(nsamples_min_vaf = n()) %>%
                           dplyr::select(key, nsamples_min_vaf) %>%
                           distinct(), by=c("key"="key")) %>%
    mutate(nsamples_min_vaf = ifelse(is.na(nsamples_min_vaf), 0, nsamples_min_vaf))
  
  df <- df %>% left_join(., df %>% 
                           dplyr::filter(average_AD >= n_samples_min_count) %>%
                           group_by(key) %>%
                           mutate(nsamples_min_count = n()) %>%
  ...
  df <- df %>% dplyr::filter(nsamples_min_vaf < count_threshold | (key == 'chr20 32434638 A>AG' & average_AF >= 0.05) | (key == 'chr20 32434638 A>AGG' & average_AF >= 0.05))        # Recurrent Filter
  ...
  df <- df %>% dplyr::filter(!(nsamples_min_vaf > bb_count_threshold & (sourcetotalsc_XGB <= 25 & CosmicCount <= 50)))
...
AddNearColumns <- function(df) {
  ...
  df$aa.pos <- as.numeric(str_extract(df$AAchange.x, "\\d+"))
  df$gene_aachange <- with(df, paste(SYMBOL_VEP, AAchange.x,sep=":"))
  
  df$near.BB.HS <- apply(df[,c("CHROM","POS","SYMBOL_VEP","aa.pos","gene_aachange", "gene_cDNAchange")], 1, function(x) {
    p = c(-3:0,1:3)
    n = c(-9:0,1:9)
  ...
  # toJSON
  df$near.BB.HS <- sapply(df$near.BB.HS, function(x) {
    ifelse (x == "", return(x), return(toJSON(x)))
  })
  df$near.BB.HS <- ifelse(grepl(",", df$near.BB.HS), df$near.BB.HS, "")
  ...
  df$near.heme.cosmic.HS <- apply(df[,c("CHROM","POS","SYMBOL_VEP","aa.pos","gene_aachange","gene_cDNAchange")], 1, function(x) {
    p = c(-3:0,1:3)
    n = c(-9:0,1:9)
  ...
  # toJSON
  df$near.heme.cosmic.HS <- sapply(df$near.heme.cosmic.HS, function(x) {
    ifelse (x == "", return(x), return(toJSON(x)))
  })
  
  df$near.BB.loci.HS <- apply(df[,c("CHROM","POS","SYMBOL_VEP","aa.pos","gene_aachange", "gene_cDNAchange")], 1, function(x) {
    p = c(-3:0,1:3)
    n = c(-9:0,1:9)
  ...
  # toJSON
  df$near.BB.loci.HS <- sapply(df$near.BB.loci.HS, function(x) {
    ifelse (x == "", return(x), return(toJSON(x)))
  })
  df$near.BB.loci.HS <- ifelse(grepl(",", df$near.BB.loci.HS), df$near.BB.loci.HS, "")
  
  df$near.heme.cosmic.loci.HS <- apply(df[,c("CHROM","POS","SYMBOL_VEP","aa.pos","gene_aachange","gene_cDNAchange")], 1, function(x) {
    p = c(-3:0,1:3)
    n = c(-9:0,1:9)
  ...
  # toJSON
  df$near.heme.cosmic.loci.HS <- sapply(df$near.heme.cosmic.loci.HS, function(x) {
    ifelse (x == "", return(x), return(toJSON(x)))
  })
  
  
  df$near.heme.cosmic.loci.HS.logic <- unlist(lapply(df$near.heme.cosmic.loci.HS, function(x) {
    if (x == "") {
      return(FALSE)
    } else {
      near <- fromJSON(x)
  ...
  df$near.BB.loci.HS.logic <- unlist(lapply(df$near.BB.loci.HS, function(x) {
    total_source <- 0
    if (x == "") {
      return(FALSE)
    } else {
  ...
  if (!("n.loci.truncating.vep" %in% colnames(df))){
    df$truncating <- ifelse(grepl("Ter",final$AAchange.x), "truncating", "not")
    dims <- dim(df)[[1]]
    df <- sqldf("SELECT l.*, r.`n.loci.truncating.vep`, r.`source.totals.loci.truncating`, r.`truncating`
             FROM `df` as l
             LEFT JOIN `vars` as r
             on (l.key = r.key AND l.truncating = r.truncating) OR (l.gene_loci_vep = r.gene_loci_vep AND l.truncating = r.truncating)")
    df <- df[!duplicated(df),]
    paste0("dims match after sqldf: ",dim(df)[[1]] == dims)
    df <- df %>% dplyr::select(-truncating)
  }
}

ProportionalRecurrentFilter <- function(df) {
  min_samples_for_recurrent <- max(ceiling(length(unique((df$SN_TAG)))*0.001), 2)  # It's 5 in UKBB
  total_samples = length(unique(df$SN_TAG)) # 1934 (for Archer)
  total_heme_cosmic = 29234
  total_BB = 122691
  
  # Ratios are Calculated Manually
  # There needs to be B/B Count and/or Cosmic Count
  # The minimum ratio before a real variant is the cut-off
  # The cut-off can be different for B/B and COSMIC
  ratio_to_cosmic_cut_off <- 835
  ratio_to_BB_cut_off <- 1335
  
  df <- df %>% mutate(prop_nsamples = nsamples_min_vaf/total_samples, 
  ...
  df <- df %>% mutate(pass_prop_recurrent = case_when(
    ...
    nsamples_min_vaf > min_samples_for_recurrent & sourcetotalsc_XGB == 0 & heme_cosmic_count != 0 & ratio_to_cosmic > ratio_to_cosmic_cut_off ~ FALSE,
    nsamples_min_vaf > min_samples_for_recurrent & sourcetotalsc_XGB != 0 & heme_cosmic_count == 0 & ratio_to_BB > ratio_to_BB_cut_off ~ FALSE,
    nsamples_min_vaf > min_samples_for_recurrent & sourcetotalsc_XGB != 0 & heme_cosmic_count != 0 & (ratio_to_BB > ratio_to_BB_cut_off & ratio_to_cosmic > ratio_to_cosmic_cut_off) ~ FALSE,
    TRUE ~ TRUE
...
PutativeDriver <- function(df) {
  ...
  df <- df %>% mutate(putative_driver = case_when(
  ...
  df <- df %>% mutate(putative_driver = ifelse(grepl("Oncogenic", oncoKB) & Consequence_VEP == "splice_region_variant&synonymous_variant" & !(clinvar_CLINSIGN_VEP %in% clinvar_sig_terms), 0, putative_driver),
...
Review <- function(df) {
  df <- df %>% mutate(Review = "No Review")
  
  # Review Rule for SRSF2 for Matthew Walters
  # Need to split Protein_position_VEP up into beginning and end
  df <- df %>% mutate(Protein_position_VEP_start = ifelse(grepl("-", Protein_position_VEP), str_split(Protein_position_VEP,"-")[[1]][1], Protein_position_VEP),
  ...
  df <- df %>% mutate(Review = ifelse(CALL_BY_CALLER == "mutect" & (average_AF >= 0.01 & average_AF < 0.02), "LowVAF Mutect", Review))
  df <- df %>% mutate(Review = case_when(
  ...
  df$MAX_PINDEL_MATCH <- unlist(lapply(df$PINDEL_MATCH, function(x) {
    if (x == "") {
      return(0)
    } else {
